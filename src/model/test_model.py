...
class SingleTester():
    ...
    def __init__(self, config: dict):
        ...
        self.ext_dataset, self.ext_mut = tr.get_dataset(
            os.path.join(self.data_dir, config['VALID']['VALID_DATA_PATH']),
            os.path.join(self.data_dir, config['VALID']['VALID_ID_PATH']),
        )

        if tr.get_data_format(config['MODEL']['MODEL_TYPE']) == 'rapsix':
            self.ext_dataset = tr.attach_snvbox(
                self.ext_dataset,
                os.path.join(self.data_dir, config['VALID']['VALID_SNVBOX']),
            )
    ...
    def get_test_set(self):
        ...
        if config['VALID']['USE_TEST_ID'][0]: # when using samples of test_ids among the dataset
            test_ids = np.load(
                os.path.join(config['log_dir'], 'test_ids.npy')
            )
    ...
    def get_test_pred(self, model_idx: int, test_set: tuple, test_ids: np.array) -> np.array:
        ...
        with open(
            os.path.join(
                config['log_dir'],
                f"{valid_name}_{model_idx}_ours.txt"
            ), 'w'
        ) as score_file:
    ...
    def save_performance(self, test_set: np.array, test_pred: np.array, test_metrics: list, \
        ...
        if test_metrics: # skip when test_metrics == []
            result_list = self.model.evaluate(
                test_set[2],
                test_pred,
                metrics=test_metrics,
                out_path=os.path.join(
                    self.config['log_dir'],
                    f"{valid_name}_{model_idx}.txt",
                )
            )
        return result_list
...
def main():
    ...
    logging.basicConfig(filename='test_model.py.log',
